...
COM_segments = [G_1; G_2; G_3; G_4; G_5];
COM_fcn = matlabFunction(COM_segments,'File','COM_segments_fcn.m');
...
COM = (G_1*m1 + G_2*m2 + G_3*m3 + G_4*m4 + G_5*m5)/(m1 + m2 + m3 + m4 + m5);
COM_fcn = matlabFunction(COM,'File','COM_fcn.m');
COMvel = (dG_1*m1 + dG_1*m2 + dG_1*m3 + dG_1*m4 + G_5*m5)/(m1 + m2 + m3 + m4 + m5);
COMvel_fcn = matlabFunction(COMvel,'File','COMvel_fcn.m');
% Get accelerations
